...
![](man/images/Athena_download_box.png)
...
![](man/images/Sepsis_to_SNOMED.png)
...
* [ACHILLES](https://github.com/OHDSI/Achilles) – a stand-alone tool for database characterization
* [ATLAS](https://www.ohdsi.org/web/atlas/#/home) - an integrated platform for vocabulary exploration, cohort definition, case review, clinical characterization, incidence estimation, population-level effect estimation design, and patient-level prediction design ([link to github](https://github.com/OHDSI/Atlas))
...
![](man/images/ATLAS_Persistence_Window.png)
...
- Recommend
  ...
  - ![](man/images/AWS_clusters.png)

## How is OMOP CDM versioning handled?

The OHDSI Common Data Model follows a modified Semantic Versioning scheme (`x.y.z`):

* **Major Releases (`x.0.0`)**: Include breaking schema changes, such as dropping or renaming tables and columns, changing data types, or major architectural restructuring. These releases require ETL code modifications.
* **Minor Releases (`x.y.0`)**: Include backwards-compatible schema additions, such as introducing new optional tables (e.g., `EPISODE`) or adding nullable columns to existing tables.
* **Patch / Build Releases (`x.y.z`)**: Strictly **disallow structural database schema changes** (no adding, removing, or renaming tables or columns). Patch releases are reserved for SQL dialect DDL fixes, constraint/index syntax updates, Quarto documentation fixes, website updates, and R package tooling maintenance.

Note: Site documentation pages align with minor versions (`x.y`), while patch updates (`x.y.z`) update repository release tags, DDL files, and `NEWS.md`.
